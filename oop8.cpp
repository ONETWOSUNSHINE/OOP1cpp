...
public:
	...
	void push_packFirst(T* const& value)
	{
		head = afterTail = new Element(value);
		head->next = nullptr;
		afterTail->prev = nullptr;
	}
...
int main()
{
	...
	//std::cout << tasks.first()->toString() << std::endl;
	//BinaryArithmeticTask* binarytask = new BinaryArithmeticTask("Addition", 5, 3, Addition);
	//binarytask->execute();
	//std::cout<< binarytask->toString() << std::endl;
 ...
 //   clearObjectsTask->execute();
	...
	//for (int i = 2; i <= 7; ++i)
	//{
	//	s.add_back(new int(i));
	//}
	//for (int i = 2; i <= 7; ++i)
	//{
	//	t.add_front(new int(i));
	//}
	//s.print();
	//s.remove_back();
	//s.print();
